...
def conditional_process_temp_frame(
    ...
    if target_vision_frame is None:
        print(f"Failed to read target vision frame from {temp_frame_path}")
        return False
    ...
    # Validate reference frame as well
    if reference_vision_frame is None:
        print(
            f"Failed to read reference vision frame for {state_manager.get_item('target_path')}"
        )
        return False
    ...
    for processor_module in get_processors_modules(
        ...
        # Ensure temp_vision_frame and temp_vision_mask are valid before slicing
        if temp_vision_frame is None:
            print("Temporary vision frame is None, skipping processors")
            return False
        if temp_vision_mask is None:
            print("Temporary vision mask is None, skipping processors")
            return False
