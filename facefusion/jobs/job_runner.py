...
def run_step(
    ...
    if job_manager.set_step_status(job_id, step_index, "started") and process_step(
        ...
        return move_file(step_output_path, output_path) and job_manager.set_step_status(
...
def finalize_steps(job_id: str) -> bool:
    ...
    for output_path, temp_output_paths in output_set.items():
        ...
        if are_images(temp_output_paths):
            for i, temp_output_path in enumerate(temp_output_paths):
                if i == 0:
                    ...
                    if "." in output_path:
                        ...
                        dest_path = f"{output_path}-{i}"
                    if not move_file(temp_output_path, dest_path):
                        return False
    return True
